...
Si vamos al usuario system, los cambios no se ven reflejados. Esto se debe a que no se ha realizado un commit


Para solucionar esto... hay que:
- volver al usuario digitador
- Y aplicar un commit

Se puede utilizar el tarrito con icono verde o escribir *commit*

> [!IMPORTANT]
> Cuando un usuario no es dueño de una tabla, despues de hacer cambios (INSERT, DELETE, UPDATE, ETC) debe hacer un ***commit*** para que se reflejen los cambios.

```SQL
-- ESTO NO SE EJECUTA Y LANZA ERROR, YA QUE NO TIENE PERMISOS PARA HACER UPDATE NI PARA LA TABLA CLIENTEDESTINO
UPDATE SYSTEM.CLIENTEDESTINO
SET FONOCLI='964579822'
WHERE RUTCLIENTE='20900289-2';
```

En usuario system hay que crear otro rol que permitira hacer cambios en una tabla

```SQL
-- CREAR ROL PARA HACER CAMBIOS EN UNA TABLA
CREATE ROLE CAMBIOS;

-- AGREGAR PRIVILEGIOS
GRANT SELECT, UPDATE, DELETE ON CLIENTEDESTINO TO CAMBIOS;

-- ASIGNAR PRIVILEGIOS A CAJERO
GRANT CAMBIOS TO CAJERO;

-- AHORA FUNCIONA PORQUE CAJERO YA TIENE PERMISOS

SELECT * FROM SYSTEM.CLIENTEDESTINO;
...
## Privilegios para crear tablas y vistas

Crearemos un usuario auditor que podrá crear tablas y vistas de la tabla encomiendas y tendrá el rol ingresos.

```sql

CREATE USER AUDITOR IDENTIFIED BY PWAUDI
DEFAULT TABLESPACE USERS
QUOTA UNLIMITED ON USERS;

GRANT CONNECT, CREATE TABLE, CREATE VIEW TO AUDITOR;

GRANT INGRESOS TO AUDITOR;


-- CONECTARSE COMO AUDITOR

-- CREA UNA COPIA DE LA TABLA ENCOMIENDAS
CREATE TABLE ENCO2 AS SELECT * FROM SYSTEM.ENCOMIENDAS;

CREATE VIEW VISTAENCO AS SELECT * FROM ENCO2 WHERE COSTO<30000;

SELECT * FROM VISTAENCO;

```

## Eliminar o quitar permisos
Con el usuario system:

### Revoke
Para eliminar permisos o privilegios se utiliza la clausula ***revoke***


```sql
-- QUITAR ROLE DE INGRESO AL DIGITADOR
REVOKE INGRESOS FROM DIGITADOR;

-- PARA CONFIRMAR EN SYSTEM
SELECT * FROM DBA_ROLE_PRIVS WHERE GRANTEE='DIGITADOR'; -- ARROJA SOLO CONNECT PERO NO INGRESOS

-- EN HOJA DE DIGITADOR SI EJECUTAMOS
SELECT * FROM SYSTEM.ENCOMIENDAS;
-- ARROJA ERROR, YA QUE LE REVOCAMOS LOS PERMISOS

```

Si quiero eliminar un role Ingresos, le voy a quitar el role a auditor. 

> [!IMPORTANT]
> Si borramos algún role vamos a afectar a todos los usuarios que lo están utilizando, ya que si alguien lo tiene y borramos el role, va a quedar sin permiso.

```SQL
-- AHORA ELIMINAREMOS EL ROL INGRESOS CON:
DROP ROLE INGRESOS;

-- EL USUARIO AUDITOR TENIA EL ROL INGRESO, PERO COMO SE ELIMINO EL ROLE YA NO LO TIENE
SELECT * FROM DBA_ROLE_PRIVS WHERE GRANTEE='AUDITOR'; -- ARROJA SOLO CONNECT PERO NO INGRESOS

-- SI EJECUTO ESTO NO ME DEJA, YA QUE EL ROLE YA NO EXISTE
GRANT INGRESOS TO DIGITADOR;
```
